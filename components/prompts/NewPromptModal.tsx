...
interface NewPromptModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (name: string, text: string, tags: string[]) => Promise<void>;
}
...
const NewPromptModal: React.FC<NewPromptModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onSave(promptName, promptText, tagsArray);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No se pudo guardar el prompt.');
      setIsSaving(false); // only set to false on error, success is handled by parent
    }
  };
    ...
    >
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-brand-border">
          <h2 className="text-lg font-bold text-brand-text">Crear Nuevo Prompt</h2>
          <button 
            onClick={onClose}
        ...
        <main className="p-6 space-y-4">
          <div>
            <label htmlFor="prompt-name" className="block text-sm font-medium text-brand-text-secondary mb-2">
          ...
          <div>
            <label htmlFor="prompt-content" className="block text-sm font-medium text-brand-text-secondary mb-2">
           ...
           <div>
            <label htmlFor="prompt-tags" className="block text-sm font-medium text-brand-text-secondary mb-2">
