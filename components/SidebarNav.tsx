...
interface SidebarNavProps {
    user: User;
    currentView: 'inbox' | 'cases' | 'users' | 'assignments';
    onNavigate: (view: 'inbox' | 'cases' | 'users' | 'assignments') => void;
}
...
const AssignmentIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className || "w-6 h-6"}>
    ...
    return (
        <aside className="w-24 border-r flex flex-col items-center justify-between p-2 animate-fade-in" style={{ 
            ...
            <nav className="w-full space-y-2">
                ...
                {isAdmin && (
                    <>
                        <NavButton label="Asignaciones" isActive={currentView === 'assignments'} onClick={() => onNavigate('assignments')}>
